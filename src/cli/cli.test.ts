...
describe(Cli.name, () => {
  ...
  it('should pass skipAutogeneratedFileComment to the generator', async () => {
    const output = await new Cli().run({
      config: {
        dialect: 'sqlite',
        logLevel: 'silent',
    ...
    expect(output).toStrictEqual('export interface DB {}\n');
  });
  ...
  it('should parse options correctly', () => {
    ...
    assert(['--numeric-parser=number-or-string'], {
    ...
    assert(['--runtime-enums'], { runtimeEnums: true });
    assert(['--runtime-enums=false'], { runtimeEnums: false });
    assert(['--runtime-enums=pascal-case'], { runtimeEnums: 'pascal-case' });
  ...
  it('should throw an error if a CLI option has an invalid value', () => {
    const assert = (args: string[], message: string) => {
      expect(() =>
        new Cli().parseOptions(args, { config: {}, silent: true }),
      ).toThrow(message);
    };
    ...
    assert(
      ['--date-strings=bogus'],
      "Parameter '--date-strings' must be true, false, or one or more of: date, datetime, timestamp",
    );
    assert(
      ['--date-strings=date', '--date-strings=bogus'],
      "Parameter '--date-strings' must be true, false, or one or more of: date, datetime, timestamp",
    );
    assert(
      ['--dialect=bogus'],
